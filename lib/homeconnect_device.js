...
module.exports = class HomeConnectDevice extends EventEmitter {
    ...
    constructor(log, api, ha) {
        ...
        this.on('BSH.Common.Status.OperationState',
                item => this.readyImpliesPowerOn(item));
    ...
    // Workaround some appliances not reliably indicating their power state
    readyImpliesPowerOn(item) {
        // Only interested in Ready
        if (item.value != 'BSH.Common.EnumType.OperationState.Ready') return;

        // Check that the power state is not already On
        let powerState = this.getItem('BSH.Common.Setting.PowerState');
        if (powerState && powerState != 'BSH.Common.EnumType.PowerState.On') {
            // Fake the power state
