...
module.exports = {
    ...
    async addLightIfSupported(type, keys) {
        ...
        // Remember the initial state of the light
        await this.device.waitConnected(true);
        let initialOn = this.device.getItem(keys.on);

        // Some settings are not visible unless the light is switched on
        let settings = {};
        try {
            // Switch the light on, if necessary
            if (!initialOn) {
                this.warn('Switching ' + type + ' on to read settings');
                await this.device.setSetting(keys.on, true);
            }

            // Try to read the details for each of the supported settings
            for (let tag of Object.keys(keys)) {
                let key = keys[tag];
                if (allSettings.some(setting => setting.key == key))
                    settings[tag] = await this.getCached(type + ' ' + tag,
                                             () => this.device.getSetting(key));
            }
        } finally {
            // Best-effort attempt to restore the original light state
            if (this.device.getItem(keys.on) != initialOn) {
                try { await this.device.setSetting(keys.on, initialOn); }
                catch (err) {}
            }
        }
