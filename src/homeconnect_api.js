...
const REFRESH_RETRY_DELAY = 6;  // (seconds)
    ...
    async authoriseClient() {
        for (;;) {
            try {
                ...
                let retryIn = Math.max(this.savedAuth[this.clientID]
                                       ? REFRESH_RETRY_DELAY * MS
                                       : AUTH_RETRY_DELAY * MS,
                                       this.earliestRetry - Date.now());
                this.warn('Retrying client authentication in '
                          + Math.floor(retryIn / MS) + ' seconds');
