    ...
    tokenSave(token) {
        let truncate =
            s => s.substring(0, 4) + '...' + s.substring(s.length - 8)
                 + ' (' + s.length + ' characters)';
        this.log('Refresh token ' + truncate(token.refresh_token));
        this.log('Access token  ' + truncate(token.access_token)
    ...
    async getEventsRaw(haid) {
        ...
        while (true) {
            ...
            try {
                ...
                options.headers['authorization'] = this.getAuthorisation();
                let event = {};
                await this.requestStream(options, line => {
                    if (!line) {
                        // End of event, so issue a notification
                        if (!started) {
                            this.log('Started ' + description);
                ...
                this.log('Terminated ' + description + ' without error');
                dispatch({ event: 'STOP' });
            } catch (err) {
                this.log('Terminated ' + description + ': ' + err.message);
                dispatch({ event: 'STOP', err: err.retry ? null : err });
            }
        }
    }

    // Wrap a Home Connect request stream in a Promise
    requestStream(options, callback) {
