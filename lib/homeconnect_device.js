...
module.exports = class HomeConnectDevice extends EventEmitter {
    ...
    async getAvailablePrograms() {
        try {
            this.requireMonitor();
            return await this.api.getAvailablePrograms(this.haId);
        } catch (err) {
    ...
    async getAvailableProgram(programKey) {
        try {
            this.requireMonitor();
            return await this.api.getAvailableProgram(this.haId, programKey);
        } catch (err) {
    ...
    eventListener(event) {
        ...
        switch (event.event) {
        case 'START':
            this.log('Unhandled START event');
            break;
        case 'STOP':
            if (event.err) this.reportError(event.err, 'Event stream');
            this.log('Unhandled STOP event');
            break;
        case 'PAIRED':
            this.log('Unhandled PAIRED event');
            break;
        case 'DEPAIRED':
            this.log('Unhandled DEPAIRED event');
            break;
        case 'CONNECTED':
